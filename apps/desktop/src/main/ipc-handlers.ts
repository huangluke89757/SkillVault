...
const DETACHED_AGENT_COPIES_DIR = path.join(home, ".agents", "skillvault-detached")
const SKILLVAULT_BACKUPS_DIR = path.join(home, ".agents", "skillvault-backups")
...
interface SkillVaultArchiveManifest {
  format: "skillvault-migration"
  version: 1
  exportedAt: string
  skills: Array<{
    name: string
...
function parseSkillVaultManifest(zip: AdmZip): SkillVaultArchiveManifest {
  const manifestEntry = zip.getEntry("skillvault-manifest.json")
  if (!manifestEntry) {
    throw new Error("这不是有效的 SkillVault 迁移包")
  }

  const value = JSON.parse(manifestEntry.getData().toString("utf-8")) as Partial<SkillVaultArchiveManifest>
  if (
    value.format !== "skillvault-migration" ||
    value.version !== 1 ||
...
const netFetch = net.fetch.bind(net) as unknown as typeof fetch

export function registerIpcHandlers(): void {
  console.log("[ipc] registerIpcHandlers initialized")
  void cleanupDownloadTempDirectories(os.tmpdir()).catch((error) => {
    console.warn("[github-download] failed to clean stale downloads", error)
  })
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const options = {
        title: "导出 SkillVault 迁移包",
        defaultPath: path.join(app.getPath("downloads"), `SkillVault-${date}.skillvault`),
        filters: [{ name: "SkillVault 迁移包", extensions: ["skillvault"] }],
      ...
      const manifest: SkillVaultArchiveManifest = {
        format: "skillvault-migration",
        version: 1,
  ...
  ipcMain.handle("skills:inspect-import-package", async () => {
    ...
    const manifest = parseSkillVaultManifest(zip)
  ...
  ipcMain.handle("skills:import-package", async (_event, archivePath: string) => {
    if (
      ...
      path.extname(archivePath).toLowerCase() !== ".skillvault" ||
      !(await fileExists(archivePath))
    ) {
      throw new Error("请选择有效的 SkillVault 迁移包")
    }
    ...
    const tempRoot = await fs.mkdtemp(path.join(path.dirname(CANONICAL_SKILLS_DIR), ".skillvault-import-"))
    let imported = 0
    let skipped = 0
    let adapted = 0
...
`).trimEnd() + "\n"
  ...
  ipcMain.handle("skills:update", async (_event, name: string) => {
    ...
    if (parsed.type === "github") {
      tmpDir = path.join(os.tmpdir(), `skillvault-upd-${Date.now()}-${crypto.randomUUID().slice(0, 8)}`)
  ...
  ipcMain.handle(
    ...
    async (_event, skillName: string, agentName: string, agentPath: string) => {
      ...
      return syncAgentCopyToMaster({
        ...
        backupRoot: SKILLVAULT_BACKUPS_DIR,
      })
    },
  )
}
