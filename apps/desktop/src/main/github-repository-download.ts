...
export async function cleanupDownloadTempDirectories(tempRoot: string): Promise<void> {
  const entries = await fs.readdir(tempRoot, { withFileTypes: true }).catch(() => [])
  await Promise.all(entries
    .filter((entry) =>
      entry.isDirectory() && /^skillvault-\d{10,}-[0-9a-f]{8}$/i.test(entry.name),
    )
    .map((entry) =>
...
export async function acquireGitHubRepository({
  ...
  if (skillId) {
    try {
      ...
      console.warn(
        `[github-download] Direct skill download failed for ${owner}/${repo}/${skillId}; falling back to repository download`,
        error,
      )
  ...
  const tryArchive = async (): Promise<boolean> => {
    try {
      ...
      console.warn(
        `[github-download] GitHub archive download failed for ${owner}/${repo}`,
        error,
      )
      await fs.rm(destination, { recursive: true, force: true }).catch(() => {})
      return false
  ...
  console.warn(
    directDownloadFailed
      ? `[github-download] Git clone also failed for ${owner}/${repo}`
      : `[github-download] Git clone failed for ${owner}/${repo}; falling back to GitHub archive`,
    cloneResult.error,
  )
  await fs.rm(destination, { recursive: true, force: true }).catch(() => {})
