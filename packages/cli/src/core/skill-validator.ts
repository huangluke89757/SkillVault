...
import type { ValidationResult, ParsedSkill, SizeCheckResult, DirectoryValidationResult } from "../types.js";
...
export async function validateDirectory(dir: string): Promise<DirectoryValidationResult> {
  ...
  if (!hasSkillMd) {
    return {
      valid: false,
      skillName: null,
      errors: [`No SKILL.md found in ${dir}. Create a SKILL.md file with 'name' and 'description' frontmatter.`],
    };
  }
