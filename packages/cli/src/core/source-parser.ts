...
export function parseSource(source: string): ParsedSource {
  ...
  // 4. Reject @username/slug (SkillVault registry removed)
  if (isSkillVaultSlug(source)) {
    throw new SourceParseError(
      "SkillVault registry is no longer available. Install from GitHub instead: skillvault add owner/repo",
    );
  }
...
function isSkillVaultSlug(input: string): boolean {
