...
async function addMcpToTool(tool: McpToolConfig): Promise<"added" | "exists" | "error"> {
  try {
    ...
    if (config.mcpServers.skillvault) {
      return "exists";
    }

    config.mcpServers.skillvault = MCP_ENTRY;
...
async function removeMcpFromTool(tool: McpToolConfig): Promise<"removed" | "not_found" | "error"> {
  try {
    ...
    delete config.mcpServers.skillvault;
...
export async function runSetup(args: string[]): Promise<void> {
  ...
  p.intro(fmt.bold(isRemove ? "Remove SkillVault MCP" : "Configure SkillVault MCP"));
  ...
  if (detected.length === 0) {
    ...
    p.log.info(pc.dim("You can manually add MCP config. See: skillvault --help"));
    p.outro("");
    return;
  }
  ...
  if (isRemove) {
    const confirm = await p.confirm({
      message: `Remove SkillVault MCP from ${detected.length} tool(s)?`,
    });
  ...
  for (const tool of detected) {
    ...
    if (result === "added") {
      p.log.success(`Added SkillVault MCP to ${tool.name}`);
  ...
  console.log();
  p.log.info(`${pc.bold("CLI:")}  skillvault add owner/repo`);
