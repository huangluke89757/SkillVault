...
<p align="center">
  <img src="docs/images/skillbox-market.png" width="100%" alt="SkillVault 90,000+ Skill 市场：自然语言意图匹配、热门排行与搜索" />
</p>
<p align="center"><sub>Skill 市场：支持自然语言意图匹配、热门排行，以及按名称 / 作者搜索，一键安装到指定 Agent。</sub></p>

## AI 意图匹配市场

除了按名称 / 作者搜索，你也可以用**自然语言**描述需求，由本地语义模型自动从仓库中挑出最相关的技能。

- 打开「发现」页，**意图匹配就在页面上方、热门排行在下方，二者同屏**；
- 用一句话描述你想做的事（例如「帮我写单元测试」「把这段 SQL 优化一下」「生成一个 React 组件」），也可以附一张图片作为参考；
- 系统返回 Top-K 匹配技能，每张卡片给出「是什么 / 有什么用 / 怎么用」三段介绍、**提示词用法**与**一键安装**；
- 结果上方还会给出 Top-3 **技能组合方案**，把多个技能串成一条协作流程；单技能与组合方案都支持**一键复制提示词**，复制后直接粘进对话即可；
- 技能详情弹窗的介绍支持**中英双语切换**（默认中文）；
...
- 「发现」页 Skill 市场改版：意图匹配与热门排行**同屏呈现**，不再用「热门 / 搜索 / 意图匹配」互斥 Tab 切换
- 右上角新增**常驻搜索输入框**，输入即进入应用内全屏搜索结果，按安装量倒序排列
- 新增**分类快捷筛选 Tab**（全部 / 开发 / 写作 / 设计 / 数据 / 效率 / 其他），常驻置顶，与官方筛选、安装目标、搜索互相独立
- 意图匹配新增 Top-3 **组合方案**与单技能**提示词用法**，均支持一键复制
- 技能详情弹窗介绍支持**中英双语切换**（默认中文）
- 补齐新增文案的英文翻译；修正文本节点被合并后无法命中词典、导致英文界面漏翻的问题
- 修复分类 Tab 初始值与筛选项不一致，导致「热门排行」被清空的问题
- 修复内容安全策略（CSP）未放行本地语义模型的下载域，导致意图匹配始终降级为关键词匹配的问题
